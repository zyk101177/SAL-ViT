...
class CCT(nn.Module):
    def __init__(self,
        ...
        #print (f"softmax_approx: {kwargs['softmax_approx']}")
        ...
        self.classifier = TransformerClassifier(
            ...
            softmax_approx=kwargs['softmax_approx'],
            attention_mechanism=kwargs['attention_mechanism'],
            externalattention_dim=kwargs['externalattention_dim'],
            externalattention_divhead=kwargs['externalattention_divhead'],
            # alpha_threshold=kwargs['alpha_threshold'],
            allargs=kwargs['allargs']
        )

    def forward(self, x):
        x = self.tokenizer(x)
        x, last_vit_feature=self.classifier(x)
...
def cct_7(arch, pretrained, progress, num_heads, *args, **kwargs):
    #print (f"kwargs['softmax_approx']: {kwargs['softmax_approx']}")
    # return _cct(arch, pretrained, progress, num_layers=7, num_heads=4, mlp_ratio=2, embedding_dim=256,
    #             *args, **kwargs)
    return _cct(arch, pretrained, progress, num_layers=7, num_heads=num_heads, mlp_ratio=2, embedding_dim=256,
            *args, **kwargs)
    #return _cct(arch, pretrained, progress, num_layers=7, num_heads=4, mlp_ratio=2, embedding_dim=256, *args, **kwargs)

def cct_9(arch, pretrained, progress, num_heads, *args, **kwargs):
    #print (f"kwargs['softmax_approx']: {kwargs['softmax_approx']}")
    return _cct(arch, pretrained, progress, num_layers=9, num_heads=num_heads, mlp_ratio=2, embedding_dim=192,
                *args, **kwargs)
...
@register_model
def cct_9_7x2_224_sine(pretrained=False, progress=False,
                       img_size=224, positional_embedding='sine', num_classes=102, num_heads=4,
                       *args, **kwargs):
    return cct_9('cct_9_7x2_224_sine', pretrained, progress,
                 kernel_size=7, n_conv_layers=2,
                 img_size=img_size, positional_embedding=positional_embedding,
