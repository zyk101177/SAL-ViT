...
def init_parser(parser):
    ...
    parser.add_argument('--n_attn_layers', default=7, type=int)
    parser.add_argument('--positional_embedding',type=str, default='sine')

    parser.add_argument('--num_heads', default=4, type=int)
    parser.add_argument('--input_N_dim', default=256, type=int)
...
def train_top_module(args):
    ...
    if args.apply_knowledge_distillation:
        if args.n_attn_layers==7:
            teacher_model_name = f'cct_7_{args.kernel_size}x{args.n_conv_layers}_{img_size}_{args.positional_embedding}'
            ...
            teacher_model_name = f'cct_14_{args.kernel_size}x{args.n_conv_layers}_{img_size}_{args.positional_embedding}'
    ...
    if random_sa_selection == False and fix_sa_selection == None:
        ...
        for name, param in model.named_parameters():
            if 'alphas' in name:
                alphas_dic[name] = param.data.tolist()
        
        # print('before sorting: ', alphas_dic)
        alphas_sorted = sorted(alphas_dic.items(), key=lambda x:x[1], reverse=True)
        # print('after sorting', alphas_sorted)

        alphas_sorted = alphas_sorted[0:args.self_attn_limit]
        self_attn_names = [item[0] for item in alphas_sorted]
        # print(self_attn_names)

        for name, param in model.named_parameters():
            if 'alphas' in name:
                if name in self_attn_names:
    ...
    for epoch in range(finetune_epoch):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(model, f"./best_model/layerwise_vit_approx_{args.softmax_approx}_dataset_{args.dataset}_attention_{args.attention_mechanism}_self_attn_limit_{args.self_attn_limit}_randomSAselect_{random_sa_selection}_fixSALocation_{fix_sa_selection}_NASepoch_{args.NAS_epoch}_numHead_{args.num_heads}.pth")
            # torch.save(
            #     {
            ...
            #     f"./best_model/best_student_approx_vit_KD_{args.apply_knowledge_distillation}_pretrain_{args.load_pretrain_model}_approx_{args.softmax_approx}_dataset_{args.dataset}_imgsize_{img_size}_bs_{args.batch_size}_attention_{args.attention_mechanism}_arch_{args.kernel_size}x{args.n_conv_layers}_{args.n_attn_layers}.pth",
            # )
